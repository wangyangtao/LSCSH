        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            centerWeight_batch = self.useGPU(
                torch.tensor(centerWeight[i * self.option.batch_size:(i + 1) * self.option.batch_size],
                             requires_grad=True))
            hash_center = self.getHashCenters(target, hash_center_pre.detach(), centerWeight_batch)

            if not self.option.fixed_weight:
                optimizer.zero_grad()
                centerWeight_batch.retain_grad()
                loss = criterion(hash_code.detach(), hash_center, centerWeight_batch, target)
                loss.backward()
            ...
            optimizer.zero_grad()
            loss = criterion(hash_code, hash_center.detach(), centerWeight_batch.detach(), target)
            loss.backward()
            optimizer.step()
